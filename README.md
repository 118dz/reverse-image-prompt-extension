...
一个 Manifest V3 Chrome/Edge 扩展 MVP：右键网页图片，调用支持视觉输入的 Kimi/OpenAI-compatible 接口，提取可迁移的风格、韵味、构图语言和可直接使用的生图提示词。
...
- `background.js`：右键菜单、API URL/Key 检查、图片 URL 转 Base64、AI 接口调用、风格迁移提示词框架和进度通知。
- `popup.html` / `popup.js`：API URL、API Key、模型名称绑定、保存、清除。
- `content.js`：页面内进度条、错误提示、结果浮窗、改写和复制按钮；优先复制带主体填空位的中文模板，不会在网页里接收 API Key。
...
插件返回的 `style_prompt_cn` 是风格层，`prompt_template_cn` 是可直接复制的完整模板。

把你想生成的新内容写进模板开头的占位符：

```text
【替换为你要生成的主体/场景/动作】，后面接插件提炼出的构图、光影、色彩、材质和情绪风格
```

例如你想用某张海报的韵味生成“雨夜街角的侦探”，就把占位符改成：

```text
一位撑伞的年轻侦探站在雨夜街角，手里拿着档案袋，后面接插件提炼出的风格提示词
```

负面提示词放到生图工具的 Negative Prompt；画幅、参考图权重和风格强度按 `parameter_suggestions` 设置。
